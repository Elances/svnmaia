...
---";
	...
	$query="update svnauth_permission set permission='r', expire=\"$expire\" where expire <= NOW() and permission='w' and isrobot != 'y' ";
	mysql_query($query);
	if(mysql_affected_rows()>0)$valuechanged=true;


	//判断读权限是否过期，如果已过期，则改为无权限。
	$query="update svnauth_permission set  permission='n', expire=\"$expire\" where expire <= NOW() and permission='r' and isrobot != 'y' ";
	mysql_query($query);
	if(mysql_affected_rows()>0)$valuechanged=true;
	//生效
	if($valuechanged)
